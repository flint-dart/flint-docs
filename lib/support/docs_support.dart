...
class DocsSupport {
  ...
  Future<List<Map<String, dynamic>>> fetchExampleProjects() async {
    try {
      if (DB.isConnected) {
        final qb = QueryBuilder(table: 'example_projects')
          ..orderBy('published_at', 'DESC');
        final rows = await qb.get();
        if (rows.isNotEmpty) {
          return rows.map(toExampleViewModel).toList();
        }
      }
    } catch (_) {
      // Fallback to default in-memory examples list
    }
    return defaultExampleProjects.map(toExampleViewModel).toList();
  }
  ...
  Map<String, dynamic> toExampleViewModel(Map<String, dynamic> row) {
    final publishedAt = row['published_at']?.toString() ?? '';
    final description = row['description']?.toString() ?? '';
    final whatItDoes = row['what_it_does']?.toString() ?? '';
    final rawFeatures = row['features']?.toString() ?? '';
    final featuresList = rawFeatures
        .split(',')
        .map((f) => f.trim())
    ...
    return {
      ...
      'category': row['category']?.toString() ?? 'Full-Stack Apps',
      'pillar': row['pillar']?.toString() ?? 'fullstack',
      'badge': row['badge']?.toString() ?? 'Official Example',
      'github_url': row['github_url']?.toString() ??
          'https://github.com/flint-dart/flint/tree/main/examples',
      'demo_url': row['demo_url']?.toString() ?? '',
      'clone_command': row['clone_command']?.toString() ??
          'flint create my_app --template=fullstack',
      'features': featuresList,
      'tech_stack': row['tech_stack']?.toString() ?? 'Flint Dart',
      'complexity': row['complexity']?.toString() ?? 'Production Ready',
      'author': row['author']?.toString() ?? 'Flint Core Team',
      'icon_name': row['icon_name']?.toString() ?? 'code',
      'accent_color': row['accent_color']?.toString() ?? '#10b981',
      'published_at': publishedAt,
      'date': formatDate(publishedAt),
    };
  }
...
const defaultExampleProjects = <Map<String, dynamic>>[
  {
